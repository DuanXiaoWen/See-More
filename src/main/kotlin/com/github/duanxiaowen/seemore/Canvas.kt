...
class Canvas(private val seeMoreToolWindow:SeeMoreToolWindow ): JPanel() {
    ...
    fun toggleClickedNode(node: Node) {
        this.seeMoreToolWindow.toggleFocusedMethod(node.method)
        repaint()
    }

    fun clearClickedNodes() {
        this.seeMoreToolWindow.clearFocusedMethods()
        repaint()
    }
        ...
        this.visibleNodes.addAll(this.graph.getNodes()
                .filter { node ->
                    ...
                    val isVisibleAccessLevel = when {
                        Utils.isPublic(method) -> this.seeMoreToolWindow.isFilterAccessPublicChecked()
                        Utils.isProtected(method) -> this.seeMoreToolWindow.isFilterAccessProtectedChecked()
                        Utils.isPackageLocal(method) -> this.seeMoreToolWindow.isFilterAccessPackageLocalChecked()
                        Utils.isPrivate(method) -> this.seeMoreToolWindow.isFilterAccessPrivateChecked()
                        else -> true
                    }
                    val isExternalMethod = Utils.getSourceRoot(method.containingFile.virtualFile) == null
                    val isVisibleExternal = !isExternalMethod || this.seeMoreToolWindow.isFilterExternalChecked()
    ...
    private fun toCameraView(point: Point2D.Float): Point2D.Float {
        val canvasSize = this.seeMoreToolWindow.getCanvasSize()
    ...
    private fun isNodeHighlighted(node: Node): Boolean {
        return this.hoveredNode === node || this.seeMoreToolWindow.isFocusedMethod(node.method)
    }
    ...
    private fun getNodeBackgroundColor(node: Node): Color {
        if (this.seeMoreToolWindow.isNodeColorByAccess()) {
        ...
        } else if (this.seeMoreToolWindow.isNodeColorByClassName()) {
    ...
    private fun createNodeLabels(node: Node, signatureColor: Color, isNodeHovered: Boolean): List<Pair<String, Color>> {
        ...
        // file path
        if (this.seeMoreToolWindow.isRenderFunctionFilePath(isNodeHovered)) {
            labels.add(node.filePath to Colors.UN_HIGHLIGHTED_COLOR.color)
        }
        // package name
        if (this.seeMoreToolWindow.isRenderFunctionPackageName(isNodeHovered)) {
    ...
    private fun drawNodeLabels(graphics2D: Graphics2D, node: Node, labelColor: Color, isNodeHovered: Boolean) {
        ...
        val backgroundColor =
                if (this.seeMoreToolWindow.isQueried(node.method.name)) Colors.HIGHLIGHTED_BACKGROUND_COLOR.color
